...
import_schema() {
  ...
  if [ ! "$(${mysql} -BN -e 'show tables LIKE 'icingaweb_%';')" ]; then
    echo "* mysql database has not yet been set up, importing schema"
    $mysql < /usr/share/doc/icingaweb2/schema/mysql.schema.sql
    $mysql -e "INSERT INTO icingaweb_user (name, active, password_hash) VALUES ('admin', 1, '\$1\$iQSrnmO9\$qJmuj3DDxyawRj8OhuAKK/');"
